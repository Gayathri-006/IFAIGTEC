      ...
      <div className="container-pad" style={{
        ...
        <div ref={dropdownRef} className="hidden-mobile" style={{ display: "flex", gap: 28, alignItems: "center", height: "100%" }}>
          {NAV_ITEMS.map((item, navIdx) => {
            const isDropdownActive = activeTab === item.label;
            ...
            return (
              ...
              >
                <button
                  onClick={() => setActiveTab(isDropdownActive ? null : item.label)}
                  style={{
                    background: "none", border: "none", cursor: "pointer",
                    fontSize: 14, fontWeight: 600, color: isDropdownActive ? C.orange : "#4b4b58",
                    display: "flex", alignItems: "center", gap: 4, transition: "color 0.25s",
                    height: "100%", padding: "0 4px", fontFamily: "'Plus Jakarta Sans', sans-serif",
                    whiteSpace: "nowrap"
                  }}
                >
                  {item.label}
                ...
                <div style={{
                  ...
                  display: "flex",
                  gap: 38, // Closer column layout to eliminate dead space on the right side
                  zIndex: 1100,
                }}>
                    ...
                    <div key={idx} style={{ display: "flex", flexDirection: "column", gap: 14, minWidth: 130 }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                        {section.items.map((sub, sIdx) => (
                          ...
                          >
                            <ChevronRight size={12} color={C.orange} opacity="0" style={{ transition: "opacity 0.2s" }} />
                            {sub.name}
                          </a>
                        ))}
                      </div>
        ...
        <div className="hidden-mobile" style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <button onClick={() => setPage("login")} style={{
            background: "none", border: "none", cursor: "pointer",
            fontSize: 14, fontWeight: 600, color: "#4b4b58", padding: "10px 20px",
          ...
          <button onClick={() => setPage("register")} style={{
            ...
            fontSize: 14, fontWeight: 700, padding: "12px 24px", borderRadius: 99,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingLeft: 8 }}>
                {item.sections.map((sec) =>
                  sec.items.map((sub, sIdx) => (
                    <a key={sIdx} href="#" onClick={e => e.preventDefault()} style={{
                      fontSize: 14, fontWeight: 550, color: "#555", textDecoration: "none"
                    }}>
                      {sub.name}
                    </a>
                  ))
